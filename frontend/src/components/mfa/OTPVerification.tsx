	...
	const sendOTP = async () => {
		...
		try {
			...
			if (response.otp_demo) {
				...
				showNotification("success", `Demo OTP sent: ${response.otp_demo}`);
			} else {
		...
		<div className="space-y-6">
			...
			<div className="text-center">
				...
				<p className="text-sm text-muted">
					A 6-digit code has been sent to your registered email and phone
				</p>
			</div>
			...
			{timeLeft <= 0 && (
				<div className="text-center">
					<p className="text-sm text-error mb-3">OTP code has expired</p>
					<Button
						variant="secondary"
						size="sm"
						onClick={sendOTP}
			...
			<div className="text-center">
				<p className="text-xs text-muted">
					...
					{timeLeft > 0 ? (
						<span>
							Please wait {formatTime(timeLeft)} before requesting a new one
						</span>
					) : (
						<button
							onClick={sendOTP}
